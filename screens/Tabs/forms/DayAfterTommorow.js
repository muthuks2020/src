...
const selector = formValueSelector('dayAfterTommorow');
...
class Today extends React.Component {
  ...
  submitForm(values) {
    ...
    this.props.editForecast3(data, id, editData);
    onSubmit();
  }
  ...
  renderThresholdAlert() {
    ...
    Alert.alert(`Current total quantity (${total}) exeeds the threshold quantity (${threshold})`, 'Please discard incorrect values', [{ text: 'Ok', onPress: () => { reset() } }], { cancelable: false });
  }
    ...
    return (
      <View>
        ...
        <View style={{ backgroundColor: '#F4F4F4', padding: 25, borderRadius: 10 }}>
          ...
          {/* Inputs */}
          <FieldArray name="inputs" component={RenderInputs} reasons={reasons} groups={ranch.PlanningGroups} page={page} formName="dayAfterTommorow" />

          {/* Total  */}
          <View style={{ paddingTop: 15, borderTopColor: '#c1c1c1', borderTopWidth: 1 }}>
            <View style={{ flexDirection: "row", justifyContent: 'space-between', paddingTop: 10, alignItems: 'center' }}>
              <Text style={{ fontFamily: 'Rubik', textAlign: 'left', color: "#424242", flex: 1, }}>Total</Text>
              <Text style={{ fontFamily: 'Rubik', flex: 1, textAlign: 'right', fontWeight: '500', fontSize: 18, color: '#000' }}>{total}</Text>
        ...
        <View style={{ flexDirection: 'row', width: '100%', padding: 10 }}>
          ...
          <Button style={{ flex: 1, marginLeft: 5 }} onPress={handleSubmit(this.submitForm)} yellow radius>{i18n.t("submit")}</Button>
        </View>
      </View>
    )
  }
